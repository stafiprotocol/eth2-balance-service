...
import (
	...
	"github.com/stafiprotocol/reth/task/collector"
)

func startCollectorCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "start-collector",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logrus.Infof("collector config info:\nlogFilePath: %s\nlogLevel: %s\nstartHeight: %d\neth1Endpoint: %s\neth2Endpoint: %s",
				cfg.LogFilePath, logLevelStr, cfg.StartHeight, cfg.Eth1Endpoint, cfg.Eth2Endpoint)

			log.InitLogFile(cfg.LogFilePath + "./log_file/collector")
			//init db
			...
			t, err := task_collector.NewTask(cfg, db)
			if err != nil {
				return err
			}
			err = t.Start()
